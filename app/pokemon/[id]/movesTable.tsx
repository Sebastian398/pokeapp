  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4 justify-center text-black">
        <button onClick={() => setFilter("level-up")} className={`px-3 py-1 rounded ${filter==="level-up" ? "bg-blue-200" : "bg-gray-100"}`}>
          Nivel
        </button>
        <button onClick={() => setFilter("machine")} className={`px-3 py-1 rounded ${filter==="machine" ? "bg-blue-200" : "bg-gray-100"}`}>
          MT/MO
        </button>
        <button onClick={() => setFilter("tutor")} className={`px-3 py-1 rounded ${filter==="tutor" ? "bg-blue-200" : "bg-gray-100"}`}>
          Tutor
        </button>
        <button onClick={() => setFilter("egg")} className={`px-3 py-1 rounded ${filter==="egg" ? "bg-blue-200" : "bg-gray-100"}`}>
          Huevo
        </button>
      </div>
